...
def section(cut_me_vertices, cut_me_edges, mx, pp, pno, FILL=False, TRI=True):
    ...
    new_bm = bmesh_from_pydata(cut_me_vertices, cut_me_edges, cut_me_polygons, True)
    for ed_idx, ed in enumerate(new_bm.edges):
        ...
        vert1 = ed.verts[0]
        v1 = vert1.co * mx.transposed()
        vert2 = ed.verts[1]
        v2 = vert2.co * mx.transposed()
        ...
        if (pq-v1).length <= vec.length and (pq-v2).length <= vec.length:
            verts.append(pq)
            ed_xsect[ed.index] = len(ed_xsect)
    ...
    for f in new_bm.faces:
        ...
        ps = [ed_xsect[key.index] for key in f.edges if key in ed_xsect]
    ...
    if x_me:
        if edges and FILL:
            ...
            bmesh.ops.contextual_create(bm, fres['edges'])
            ...
            if not pols:
                ...
                bmesh.ops.contextual_create(bm, geom)
                for p in bm.faces:
                    vs = []
                    for v in p.verts:
                        vs.append(v)
