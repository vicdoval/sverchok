...
def ciruclar_intersections(net2, verts, v_len, radius):
    edges_in=[]
...
class SvVoronoiArcNode(bpy.types.Node, SverchCustomTreeNode):
    '''C2 Offset vert_line'''
    bl_idname = 'SvVoronoiArcNode'
    bl_label = 'Voronoi Arc 2D'
    bl_icon = 'FORCE_FORCE'
    ...
    def build_net(self, verts_in, v_len, radius, poligonal_inter):
        ...
        if not poligonal_inter and v_len > 1:
            ...
            ciruclar_intersections(net2, verts_in, v_len, radius)
    ...
    def get_inputs(self):
        ...
        family = list_matcher([verts_all, radius_all, vertices_all], self.list_match)

        return family

    def adjust_parameters(self, params, v_len, actual_radius, poligonal_inter):
        verts_in, _, vertices= params
        parameters = list_matcher([verts_in, vertices], self.list_match)
        net = self.build_net(verts_in, v_len, actual_radius, poligonal_inter)
    ...
    def generate_outlines(self, output_lists, params):
        ...
        for i in range(perimeter_number):
            ...
            net, parameters = self.adjust_parameters(params, v_len, actual_radius[i], poligonal_inter)
...
def register():
    bpy.utils.register_class(SvVoronoiArcNode)


def unregister():
    bpy.utils.unregister_class(SvVoronoiArcNode)
